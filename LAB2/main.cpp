...
class SocketDemo {
    static constexpr size_t MAX_NUMBER_OF_ACCESS_POINTS = 100;
    static constexpr size_t MAX_MESSAGE_RECEIVED_LENGTH = 100;

    static constexpr size_t REMOTE_PORT = 4000;
    // #if MBED_CONF_APP_USE_TLS_SOCKET
   ...
   public:
    ...
    void run() {
        ...
        while (1) {
            ...
            printf("\nTEMPERATURE = %.2f degC\n", sensor_value);
            json_str_stream << "\"temperature\": " << sensor_value << ", ";

            sensor_value = BSP_HSENSOR_ReadHumidity();
            printf("HUMIDITY    = %.2f %%\n", sensor_value);
            json_str_stream << "\"humidity\": " << sensor_value << ", ";

            sensor_value = BSP_PSENSOR_ReadPressure();
            printf("PRESSURE is = %.2f mBar\n", sensor_value);
            json_str_stream << "\"pressure\": " << sensor_value;

            BSP_MAGNETO_GetXYZ(pDataXYZ);
            printf("\nMAGNETO_X = %d\n", pDataXYZ[0]);
            printf("MAGNETO_Y = %d\n", pDataXYZ[1]);
            printf("MAGNETO_Z = %d\n", pDataXYZ[2]);
            json_str_stream << ",\"MAGNETO_X\": " << pDataXYZ[0];
            json_str_stream << ",\"MAGNETO_Y\": " << pDataXYZ[1];
            json_str_stream << ",\"MAGNETO_Z\": " << pDataXYZ[2];
            ...
            json_str_stream << ",\"GYRO_X\": " << pGyroDataXYZ[0];
            json_str_stream << ",\"GYRO_Y\": " << pGyroDataXYZ[1];
            json_str_stream << ",\"GYRO_Z\": " << pGyroDataXYZ[2];
            ...
            json_str_stream << ",\"ACCELERO_X\": " << pDataXYZ[0];
            json_str_stream << ",\"ACCELERO_Y\": " << pDataXYZ[1];
            json_str_stream << ",\"ACCELERO_Z\": " << pDataXYZ[2];
            json_str_stream << "}";
            std::string json_str = json_str_stream.str();
            std::cout << "JSON: " << json_str << std::endl;

            send_data(json_str.c_str());
   ...
   private:
    ...
    bool send_data(const char buffer[]) {
